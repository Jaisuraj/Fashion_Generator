...
const insertDBShirt = async (color, pattern, fabric, sleeve, collar, fit) => {
  ...
  var colorRating = await connection.query(
    `SELECT MAX(colorRating) from shirt_collection where color='${color}'`
  );
  console.log(colorRating);
  colorRating = colorRating[0][0]["MAX(colorRating)"] + 10;

  var patternRating = await connection.query(
    `SELECT MAX(patternRating) from shirt_collection where pattern='${pattern}'`
  );
  patternRating = patternRating[0][0]["MAX(patternRating)"] + 10;

  var fabricRating = await connection.query(
    `SELECT MAX(fabricRating) from shirt_collection where pattern='${fabric}'`
  );
  fabricRating = fabricRating[0][0]["MAX(fabricRating)"] + 10;

  var sleeveRating = await connection.query(
    `SELECT MAX(sleeveRating) from shirt_collection where pattern='${sleeve}'`
  );
  sleeveRating = sleeveRating[0][0]["MAX(sleeveRating)"] + 10;

  var collarRating = await connection.query(
    `SELECT MAX(collarRating) from shirt_collection where pattern='${collar}'`
  );
  collarRating = collarRating[0][0]["MAX(collarRating)"] + 10;

  var fitRating = await connection.query(
    `SELECT MAX(fitRating) from shirt_collection where pattern='${fit}'`
  );
  fitRating = fitRating[0][0]["MAX(fitRating)"] + 10;

  await connection.query(
    `UPDATE shirt_collection SET colorRating=${colorRating} where color='${color}'`
  );

  await connection.query(
    `UPDATE shirt_collection SET patternRating=${patternRating} where pattern='${pattern}'`
  );

  await connection.query(
    `UPDATE shirt_collection SET fabricRating=${fabricRating} where fabric='${fabric}'`
  );

  await connection.query(
    `UPDATE shirt_collection SET sleeveRating=${sleeveRating} where sleeve='${sleeve}'`
  );

  await connection.query(
    `UPDATE shirt_collection SET collarRating=${collarRating} where collar='${collar}'`
  );

  await connection.query(
    `UPDATE shirt_collection SET fitRating=${fitRating} where fit='${fit}'`
  );
...
const insertUsers = async (username, password, number, email) => {
  ...
  var m_ids = await connection.query(
...
const loginHandle = async (email, password) => {
  ...
  var dets = await connection.query(
    `SELECT * from users where email='${email}'`
  );
...
const insertDBPant = async (type, color, pattern, fabric, fit) => {
  ...
  var colorRating = await connection.query(
    `SELECT MAX(colorRating) from pant_collection where color='${color}'`
  );

  colorRating = colorRating[0][0]["MAX(colorRating)"] + 10;

  var patternRating = await connection.query(
    `SELECT MAX(patternRating) from pant_collection where pattern='${pattern}'`
  );
  console.log("pattern", patternRating);
  patternRating = patternRating[0][0]["MAX(patternRating)"] + 10;

  var fabricRating = await connection.query(
    `SELECT MAX(fabricRating) from pant_collection where fabric='${fabric}'`
  );
  console.log("fabric", fabricRating);
  fabricRating = fabricRating[0][0]["MAX(fabricRating)"] + 10;

  var typeRating = await connection.query(
    `SELECT MAX(typeRating) from pant_collection where type='${type}'`
  );
  console.log("type", typeRating);
  typeRating = typeRating[0][0]["MAX(typeRating)"] + 10;

  var fitRating = await connection.query(
    `SELECT MAX(fitRating) from pant_collection where fit='${fit}'`
  );
  console.log("fit", fitRating);
  fitRating = fitRating[0][0]["MAX(fitRating)"] + 10;

  await connection.query(
    `UPDATE pant_collection SET colorRating=${colorRating} where color='${color}'`
  );

  await connection.query(
    `UPDATE pant_collection SET patternRating=${patternRating} where pattern='${pattern}'`
  );

  await connection.query(
    `UPDATE pant_collection SET fabricRating=${fabricRating} where fabric='${fabric}'`
  );

  await connection.query(
    `UPDATE pant_collection SET typeRating=${typeRating} where fit='${type}'`
  );

  await connection.query(
    `UPDATE pant_collection SET fitRating=${fitRating} where fit='${fit}'`
  );
...
const retrieveShirt = async (id) => {
  ...
  const pieceId = await connection.query(
    `SELECT MAX(pieceId) FROM shirt_collection`
  );
  ...
  try {
    ...
    const string =
      ...
      " fit";
    const rating =
      colorR[0][0]["colorRating"] +
      patternR[0][0]["patternRating"] +
      fabricR[0][0]["fabricRating"] +
      sleeveR[0][0]["sleeveRating"] +
      collarR[0][0]["collarRating"] +
      fitR[0][0]["fitRating"];
    console.log(rating);
...
const retrievePants = async () => {
  ...
  const pieceId = await connection.query(
    `SELECT MAX(pieceId) FROM pant_collection`
  );
  var Id = pieceId;
  var maxId = Id[0][0]["MAX(pieceId)"];
  var r1 = Math.floor(Math.random() * maxId) + 1;
...
app.post("/register", async (req, res) => {
  var approval = await insertUsers(
    req.body.username,
    req.body.password,
    req.body.number,
    req.body.email
